# app.py — Capstone Secure Keyboard (GUI + 대시보드 + 패턴학습 + 실시간 인증 표시 강화)
...
class HookEngine:
    def __init__(
        ...
        # 테스트를 위해 10으로 낮춰 확인이 쉬움 (원하면 15~30)
        self.VERIFY_BATCH_SIZE = 10
    ...
    # ========= 실시간 인증 요청 =========
    def _verify_typing_pattern(self):
        ...
        try:
            self.ui_status_cb(f"검증 요청 전송 중… ({len(batch)} samples)")
            r = requests.post(
            ...
            if r.status_code == 200:
                result = r.json()
                auth_msg = "인증: " + ("성공" if result.get("is_user") else "실패")
                score = result.get("score", "N/A")
                self.ui_status_cb(f"{auth_msg} (정확도: {score})")
            else:
                try:
                    err = r.json().get("error")
                except Exception:
                    err = r.text
                self.ui_status_cb(f"인증 서버 오류: HTTP {r.status_code} - {err}")
        except Exception as e:
            self.ui_status_cb(f"인증 요청 실패: {e}")
    ...
    def _on_press(self, key):
        ...
        # ESC로 전체 후킹 종료를 막고 싶다면 아래 주석 유지
        # try:
    ...
    def _on_release(self, key):
        ...
        if code in self.down_map:
            ...
            dwell = (t_up_mono - t_down_mono) * 1000.0
            flight = (
                None
            ...
            # 실시간 인증 버퍼에 dwell/flight 데이터 추가
            if flight is not None and dwell > 0:
                with self.verify_lock:
                    self.verify_buf.append(
                        {"dwell_ms": round(dwell, 3), "flight_ms": round(flight, 3)}
                    )
                # 버퍼가 일정 크기에 도달하면 서버로 인증 요청 (별도 스레드)
                if len(self.verify_buf) >= self.VERIFY_BATCH_SIZE:
                    threading.Thread(
        ...
        # <<< 중요 >>> 모든 입력에 대해 마지막 업 타임/코드 갱신 (learn_active 여부와 무관)
...
class App:
    ...
    def on_analyze(self):
        if not self._token:
            self.set_status("분석을 위해 먼저 로그인해야 합니다.")
            return
        self.btn_analyze.config(state=DISABLED)
        self.set_status("서버에 데이터 분석을 요청하는 중입니다...")
        threading.Thread(target=self._analyze_flow, daemon=True).start()

    def _analyze_flow(self):
        try:
            headers = {"Authorization": f"Bearer {self._token}"}
            r = requests.post(
                f"{API_BASE}/api/pattern/analyze", headers=headers, timeout=30
            )
            if r.status_code == 200:
                result = r.json()
                msg = result.get("message", "분석이 완료되었습니다.")
                ...
                self.set_status(f"분석 실패: {error_msg}")
        except requests.exceptions.RequestException as e:
            self.set_status(f"분석 요청 실패: 서버에 연결할 수 없습니다. ({e})")
        except Exception as e:
            self.set_status(f"분석 중 알 수 없는 오류: {e}")
        finally:
