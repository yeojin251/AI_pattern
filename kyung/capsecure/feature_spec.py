...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["kps_local"] = 1000.0 / (df["dt_ms_clip"] + 1e-6)  # 11/4 추가

    # 국소 중앙값 정규화 (속도 무력화) 11/4 추가
    # 롤링 중앙값(윈도 50) 우선, 부족하면 expanding 중앙값으로 백업
    r_med_dw = dwell.rolling(50, min_periods=10).median()
    r_med_fl = flight.rolling(50, min_periods=10).median()
    e_med_dw = dwell.expanding().median()
    e_med_fl = flight.expanding().median()
    denom_dw = r_med_dw.fillna(e_med_dw).replace(0, np.nan).fillna(1.0)
    denom_fl = r_med_fl.fillna(e_med_fl).replace(0, np.nan).fillna(1.0)
    df["dwell_norm"] = dwell / denom_dw
    df["flight_norm"] = flight / denom_fl
