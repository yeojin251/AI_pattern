...
def train_and_save_model(username: str):
    ...
    print(f"[INFO] 7피처 생성 완료: 원본 {before_drop} → 유효 {after_drop} 행")
    print(f"[INFO] FEATURE_NAMES = {FEATURE_NAMES}")

    if len(X) < MIN_SAMPLES:
        print(f"[ERR] 유효 표본이 부족합니다 (>= {MIN_SAMPLES} 필요). 현재: {len(X)}")
        print("[HINT] 수집을 더 하고, 앱에서 '수집 데이터 분석' 버튼(Analyze)을 먼저 눌러보세요.")
        return
    ...
    if os.path.exists(profile_path):
        try:
            ...
            if scaler is None or list(prof_feats) != list(FEATURE_NAMES):
                print(f"[WARN] 기존 프로필이 7피처 스펙과 불일치 → 새로 생성합니다.")
                scaler = None
    ...
    if need_save_profile:
        ...
        print(f"[PROFILE] 저장: {profile_path} (count={profile['count']}, features={profile['feature_names']})")
    else:
        print(f"[PROFILE] 사용: {profile_path} (features={profile.get('feature_names')})")
